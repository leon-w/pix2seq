...
class ResnetBasicBlock(tf.keras.layers.Layer):
    ...
    def __init__(self, out_dim, kernel_size, dropout, **kwargs):
        super().__init__(**kwargs)
...
class TransUNet(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.up_units = {}
        self.dn_units = {}
        for i, (n_res_blocks, kernel_size, ch_multiplier) in enumerate(
            zip(config.n_res_blocks, config.kernel_sizes, config.ch_multipliers)
        ):
