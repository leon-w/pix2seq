...
def get_category_names(category_names_path: Optional[str]) -> Dict[int, Dict[str, Any]]:
...
def decode_object_seq_to_bbox(logits, pred_seq, quantization_bins, coord_vocab_shift):
    ...
    mask_s1 = [0.0] * vocab.BASE_VOCAB_SHIFT  # reserved.
    mask_s2 = [1.0] * (coord_vocab_shift - vocab.BASE_VOCAB_SHIFT)  # labels.
    ...
    pred_score = tf.reduce_sum(pred_class_p * tf.one_hot(pred_class, vocab_size), -1)
...
def compute_weighted_scores(bbox_scores, pred_seq, logits, points_score_weight):
    ...
    return points_score_weight * points_score + (1 - points_score_weight) * bbox_scores
...
def bits_to_panoptic_map(bits, n_bits_label, num_classes, max_instances_per_image):
...
def get_normalized_weight(id_map, total_num_ids, p=1.0):
    ...
    weight = 1.0 / (tf.reduce_sum(id_map_hot, [1, 2]) + 1)
