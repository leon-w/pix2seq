...
class TaskKeypointDetection(task_lib.Task):
    ...
    def preprocess_batched(self, batched_examples, training):
        ...
        response_seq_mask = tf.less(tf.random.uniform(response_seq_mask_shape), ratio)
    ...
    def infer(self, model, preprocessed_outputs):
        ...
        pred_seq, logits, _ = model.infer(  # pred_seq (bsz * instances, seqlen)
            ...
            prompt_seq,
            encoded=encoded,
            max_seq_len=config.max_points_per_object * 2 + 6,
            temperature=config.temperature,
            top_k=config.top_k,
            top_p=config.top_p,
        )
        if config.eval_suppress_invisible_token:
            ...
            offset = tf.tensor_scatter_nd_update(
                offset,
                [[i] for i in eval_suppress_tokens],
    ...
    def postprocess_tpu(
        ...
        pred_seq,
        logits,
        pred_classes,  # pytype: disable=signature-mismatch  # overriding-parameter-count-checks
        pred_bboxes,
        scores,
        training=False,
    ):
    ...
    def postprocess_cpu(
        ...
        (
            ...
            n_instances,
            pred_points,
            pred_points_rescaled,  # pylint: disable=unbalanced-tuple-unpacking
            pred_classes,
            pred_bboxes,
            scores,
        ) = new_outputs
...
def add_image_summary_with_keypoints(
    images,
    bboxes,
    keypoints,
    classes,
