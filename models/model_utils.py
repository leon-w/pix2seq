...
class WarmUpAndDecay(tf.keras.optimizers.schedules.LearningRateSchedule):
    ...
    def __init__(
        ...
        if learning_rate_scaling == "linear":
            self.base_lr = base_learning_rate * batch_size / 256.0
    ...
    def __call__(self, step):
        ...
        if schedule == "linear":
            linear_decay = tf.keras.optimizers.schedules.PolynomialDecay(
                base_lr,
                total_steps - warmup_steps - tail_steps,
                end_learning_rate=base_lr * end_lr_factor,
                power=1.0,
            )
...
def build_optimizer(config, learning_rate):
    ...
    if config.optimizer == "momentum":
        return tf.keras.optimizers.SGD(learning_rate, config.momentum, nesterov=True)
...
def get_loss(logits, label_seq, loss_type):
    ...
    if "xent" in loss_type:
        ...
        focal_weight = tf.pow(1.0 - p, gamma) if gamma > 0 else 1.0
