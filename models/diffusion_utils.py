...
class Scheduler(object):
    ...
    def add_noise(self, inputs, time_step=None):
        """Forword process."""
    ...
    def transition_step(
        ...
        if sampler_name == "ddim":
            samples = data_pred * sqrt(gamma_prev) + noise_pred * sqrt(1 - gamma_prev)
        ...
        for t in tf.range(iterations):
            ...
            if guidance == 0:
                ...
                pred_out_nl = transition_f(
                    ctx.contextualized_inputs(samples),
                    gamma,
                    training=False,
                    drop_label=True,
                )
...
def simple_linear_schedule(t, clip_min=1e-9):
    ...
    Returns:
      `float` of transformed time between 0 and 1.
    """
    return tf.clip_by_value(1.0 - t, clip_min, 1.0)
...
def get_x0_from_eps(xt, gamma, noise_pred):
    data_pred = 1.0 / sqrt(gamma) * (xt - sqrt(1.0 - gamma) * noise_pred)
    return data_pred


def get_eps_from_x0(xt, gamma, data_pred):
    noise_pred = 1.0 / sqrt(1 - gamma) * (xt - sqrt(gamma) * data_pred)
    return noise_pred
...
def get_x0_eps(xt, gamma, denoise_out, pred_type, x0_clip_fn, truncate_noise=False):
