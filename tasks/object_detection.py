...
class TaskObjectDetection(task_lib.Task):
    ...
    def postprocess_cpu(
        ...
        (
            ...
            pred_bboxes,
            pred_bboxes_rescaled,
            pred_classes,  # pylint: disable=unbalanced-tuple-unpacking
            scores,
            gt_classes,
            gt_bboxes,
            gt_bboxes_rescaled,
    ...
    def evaluate(self, summary_writer, step, eval_tag):
        ...
        if self.config.task.get("eval_outputs_json_path", None):
            ...
            json_path = self.config.task.eval_outputs_json_path.format(
                ...
                max_instances_per_image_test=self.config.task.max_instances_per_image_test,
                step=int(step),
            )
            tf.io.gfile.makedirs(os.path.basename(json_path))
            logging.info(
                "Saving %d result annotations to %s",
                len(self.eval_output_annotations),
                json_path,
            )
...
def build_response_seq_from_bbox(
    ...
    quantized_bbox = tf.where(is_padding, tf.zeros_like(quantized_bbox), quantized_bbox)
    ...
    rand_cls = vocab.BASE_VOCAB_SHIFT + tf.random.uniform(
        lb_shape, 0, coord_vocab_shift - vocab.BASE_VOCAB_SHIFT, dtype=new_label.dtype
    )
