    ...
    example_proto = tf.train.SequenceExample(
        context=tf.train.Features(
            feature={
                ...
                "video/num_frames": tf.train.Feature(
...
def main(unused_argv):
    ...
    video_names = [
        s.strip()
        for s in tf.io.gfile.GFile(
            os.path.join(data_dir, f"ImageSets/2017/{split}.txt")
        ).readlines()
    ]
    ...
    writers = [
        tf.io.TFRecordWriter(
            os.path.join(
                output_dir, f"{split}_{num_frames}-{i:05d}-of-{shards:05d}.tfrecord"
            )
        )
