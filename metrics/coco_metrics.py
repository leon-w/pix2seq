...
class _CocoNonPanopticMetric:
    ...
    def __init__(self, config, iou_type):
    ...
    def _evaluate(self):
        ...
        return dict(zip(self.metric_names, np.array(coco_metrics, dtype=np.float32)))
...
class CocoInstanceSegmentationMetric(_CocoNonPanopticMetric):
    ...
    def __init__(self, config):
        super().__init__(config, iou_type="segm")

    def record_prediction(self, image_ids, segments, classes, scores):
    ...
    def record_groundtruth(self, image_ids, segments, classes, areas):
...
class CocoKeypointDetectionMetric(_CocoNonPanopticMetric):
    ...
    def __init__(self, config):
        super().__init__(config, iou_type="keypoints")
    ...
    def record_groundtruth(
        ...
        """
        assert not self.gt_annotations_path, _CONFLICTING_ANNOTATIONS_ERROR
        (image_ids, keypoints, bbox, classes, area, iscrowd, num_keypoints) = to_numpy(
            image_ids, keypoints, bbox, classes, area, iscrowd, num_keypoints
        )
        batch_size = len(image_ids)
        for i in range(batch_size):
            if classes[i] != 0:
...
class CocoObjectDetectionMetric(_CocoNonPanopticMetric):
    ...
    def __init__(self, config):
        super().__init__(config, iou_type="bbox")

    def record_prediction(self, image_ids, pred_bboxes, pred_classes, pred_scores):
        """Records the detections and the corresponding gt data for a batch.
        ...
        Args:
          ...
          pred_scores: [batch]
        """
        (image_ids, pred_bboxes, pred_classes, pred_scores) = to_numpy(
            image_ids, pred_bboxes, pred_classes, pred_scores
        )
    ...
    def record_groundtruth(self, image_ids, bboxes, classes, areas, is_crowds):
...
class CocoCaptioningEvaluationMetric:
    ...
    def __init__(self, config):
        ...
        self.gt_annotations_path = get_annotations_path_for_metrics(config)
...
def get_annotations_path_for_metrics(
    config,
    ...
    filename = (
        ...
        if split == "train"
        else config.dataset.get("val_filename_for_metrics")
    )
