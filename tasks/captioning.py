...
class TaskCaptioning(task_lib.Task):
        ...
        if training:
            ...
            if config.input_seq_drop_rate > 0:
                input_seq = tf.where(
                    ...
                    input_seq,
                    vocab.FAKE_TEXT_TOKEN,
                )

            return batched_examples["image"], input_seq, target_seq, token_weights
        else:
            return (
                batched_examples["image"],
                batched_examples["captions"],
                batched_examples,
            )
    ...
    def postprocess_tpu(
        self, batched_examples, pred_seq, logits, training=False
    ...
    def postprocess_cpu(
        ...
        if not training:
            ...
            output_text = self._tokenizer.ids_to_strings(
                pred_seq, tf.ones([batch_size], tf.int32) * config.max_seq_len
            ).numpy()
            output_text = [o.decode("utf-8") for o in output_text]
            if self._coco_metrics:
                self._coco_metrics.record_prediction(
                    image_ids.numpy(), output_text, original_pred_seq, clipped_pred_seq
                )

            if ret_results:
                gt_seq = tf.where(gt_seq == 0, 0, gt_seq - mconfig.text_vocab_shift)
                gt_text = self._tokenizer.ids_to_strings(
                    tf.cast(gt_seq, tf.int32),
                    tf.ones([batch_size], tf.int32) * config.max_seq_len,
                ).numpy()
